...
#ifndef ALPS_TOOLS_PARAMETER_CONVERSION_HPP
#define ALPS_TOOLS_PARAMETER_CONVERSION_HPP
                    ...
                    const alps::Parameters& params_add)
{
  ...
  if (list.size() == 0) {
    ...
    if (params.size() == 0) {
      std::cerr << "Warning: no parameter set is found.  No " << Writer::name() << " files will be generated.\n";
      return;
    } else {
      std::cerr << "Info: no parameter set is found.  One task " << Writer::name() << " will be generated by using default parameters.\n";
  ...
  out << alps::start_tag("OUTPUT")
      << alps::attribute("file", basename + ".out" + Writer::extension())
      << alps::end_tag("OUTPUT");
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    out << alps::start_tag("TASK") << alps::attribute("status","new")
        ...
        << alps::attribute("file", taskname + ".in" + Writer::extension())
        << alps::end_tag("INPUT")
        << alps::start_tag("OUTPUT")
        << alps::attribute("file", taskname + ".out" + Writer::extension())
        << alps::end_tag("OUTPUT")
        << alps::end_tag("TASK");

    Writer::write(taskname + ".in" + Writer::extension(), list[i]);
  }
...
try {
  ...
  if (!force && boost::filesystem::exists(outbase + ".out.xml")) {
    std::cerr << "Output files (" + outbase + ".out.xml, etc) exist.  "
              << "Please use '-f' option to force replacing input " << Writer::name() << " files.\n"
  ...
  convert_params<Writer>(inname, outbase, params_add);
