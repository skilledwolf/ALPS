...
private:
  static constexpr bool identity_couplings = std::is_same_v<MAT, MIdMatrix<double, M::dim>>;
...
{
    if constexpr (identity_couplings) {
      if (this->cluster_updates_)
        return cluster_update(this->graph(), spins_, this->beta_, this->random_01,
                              this->couplings_, this->spinfactor_);
    }
...
void SpinSim<M,MAT>::do_measurements(update_info_type update_info)
{
  ...
  double m_h = 0.;
  // Preserve the identity-coupling specialization's first-site energy convention.
  if constexpr (!identity_couplings) {
    if (this->general_case_)
