// Copyright (C) 1997-2014 by Synge Todo and the ALPS contributors.
...
int start_sgl(int argc, char** argv) {
  ...
  using clone_proxy = typename Runtime::clone_proxy_type;
  ...
  BOOST_FOREACH(std::string const& file_str, opt.jobfiles) {
    ...
    if (Runtime::load_filename(file, file_in_str, file_out_str) != 1) {
    ...
    Runtime::load_tasks(file_in, file_out, basedir, simname, tasks, true, Runtime::task_options(opt));
    if (simname != "")
    ...
    #pragma omp parallel num_threads(num_groups)
    {
      ...
      #pragma omp master
      {
        ...
        if constexpr (Runtime::reports_vmusage) check_queue.push(next_vmusage(opt.vmusage_interval));
      } // end omp master
        ...
        while (true) {
          if (clone_ptr && clone_ptr->halted()) {
            ...
            #pragma omp critical
            {
              ...
              tasks[tid].halt_clone(proxy, Runtime::task_options(opt), cid, group);
              if (progress < 1 && tasks[tid].progress() >= 1) ++num_finished_tasks;
              Runtime::save_tasks(file_out, simname, file_in_str, file_out_str, tasks);
            } // end omp critical
            ...
            #pragma omp critical
            {
              tasks[tid].suspend_clone(proxy, Runtime::task_options(opt), cid, group);
            } // end omp critical
            ...
            if (q.type == check_type::taskinfo) {
              #pragma omp critical
              {
                ...
                for (int t = 0; t < tasks.size(); ++t) {
                  if (tasks[t].on_memory()) tasks[t].save(Runtime::task_options(opt));
                }
                std::cerr << "save task\n";
                Runtime::save_tasks(file_out, simname, file_in_str, file_out_str, tasks);
                Runtime::print_taskinfo(std::cout, tasks, opt.task_range);
        ...
        #pragma omp master
        {
          if (!process.is_halting()) {
            ...
            if (to_halt) {
              ...
              {
                for (int t = 0; t < tasks.size(); ++t)
                  tasks[t].suspend_remote_clones(proxy, Runtime::task_options(opt));
              }
              process.halt();
            }
          }
    ...
    if constexpr (Runtime::supports_evaluation) if (opt.auto_evaluate) {
...
int start_mpi(int argc, char** argv) {
  using task = typename Runtime::task_type;
  using clone_mpi = typename Runtime::clone_mpi_type;
  using clone_proxy_mpi = typename Runtime::clone_proxy_mpi_type;
  using clone_info_msg_t = typename Runtime::clone_info_msg_t_type;
  using clone_halt_msg_t = typename Runtime::clone_halt_msg_t_type;
  using clone_create_msg_t = typename Runtime::clone_create_msg_t_type;
  ...
  if (world.rank() == 0) Runtime::print_copyright(std::cout);
  ...
  if constexpr (Runtime::validate_thread_layout) {
      ...
      while (true) {
        ...
        if (world.rank() == 0) {
          ...
          if (!process.is_halting()) {
            ...
            if (to_halt) {
              BOOST_FOREACH(task& t, tasks) t.suspend_remote_clones(proxy, Runtime::task_options(opt));
              process.halt();
            }
          }
          ...
          while (!process.is_halting() || process.num_allocated() > 0) {
            ...
            if ((status = process.comm_ctrl().iprobe(boost::mpi::any_source, boost::mpi::any_tag))) {
              if (status->tag() == mcmp_tag::clone_info) {
                ...
                process.comm_ctrl().recv(Runtime::receive_source(*status), status->tag(), msg);
                if (msg.info.progress() < 1) {
                  ...
                  tasks[msg.task_id].halt_clone(proxy, Runtime::task_options(opt), msg.clone_id,
                ...
                if (tasks[msg.task_id].num_running() == 0) {
                  tasks[msg.task_id].save(Runtime::task_options(opt));
            ...
            } else if (Runtime::reports_vmusage && (status = world.iprobe(boost::mpi::any_source, boost::mpi::any_tag))) {
              ...
              double progress = tasks[tid].progress();
              tasks[tid].info_updated(cid, clone_ptr->info());
              tasks[tid].halt_clone(proxy, Runtime::task_options(opt), cid, process_group(0));
              if (progress < 1 && tasks[tid].progress() >= 1) ++num_finished_tasks;
              Runtime::save_tasks(file_out, simname, file_in_str, file_out_str, tasks);
              process.release(0);
            } else if (clone_ptr && process.is_halting()) {
              tid_t tid = clone_ptr->task_id();
              cid_t cid = clone_ptr->clone_id();
              tasks[tid].suspend_clone(proxy, Runtime::task_options(opt), cid, process_group());
              process.release(0);
		...
		suspended_queue.pop();
              ...
              if (q.type == check_type::taskinfo) {
                std::cout << logger::header() << "checkpointing task files\n";
                BOOST_FOREACH(task const& t, tasks) { if (t.on_memory()) t.save(Runtime::task_options(opt)); }
                Runtime::save_tasks(file_out, simname, file_in_str, file_out_str, tasks);
                Runtime::print_taskinfo(std::cout, tasks, opt.task_range);
                check_queue.push(next_taskinfo(opt.checkpoint_interval));
              } else if (q.type == check_type::checkpoint) {
        ...
        if (clone_ptr) {
          ...
          if (world.rank() != 0 && process.comm_ctrl().iprobe(0, mcmp_tag::clone_create)) {
            ...
            clone_ptr = Runtime::make_clone(process.comm_ctrl(), process.comm_work(), basedir, opt, msg);
          }
